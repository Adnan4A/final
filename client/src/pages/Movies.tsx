...
export default function Movies() {
  ...
  const { data: moviesData, isLoading } = useQuery({
    queryKey: ['/api/movies/popular', currentPage],
    queryFn: () => tmdbService.getPopularMovies(currentPage),
  });
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-24" data-testid="movies-page">
      <div className="container mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-4xl md:text-6xl font-orbitron font-bold mb-4 relative pl-6" data-testid="movies-title">
            <span className="relative">
              <span className="absolute -left-6 top-1/2 transform -translate-y-1/2 w-1 h-8 bg-white"></span>
              Movies
            </span>
          </h1>
          <p className="text-gray-400 text-lg" data-testid="movies-description">
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6 md:gap-8" data-testid="movies-grid">
            ...
            <Pagination
              ...
              totalPages={Math.min(moviesData.total_pages || 1, 500)} // TMDB API limit
              onPageChange={handlePageChange}
            />
          </>
        ) : (
          <div className="text-center py-12">
            <p className="text-gray-400 text-lg">No movies found at the moment.</p>
